...
项目为每个API平台优化了专属的系统提示词，确保生成质量：

| 平台 | 提示词特点 | Token消耗 |
|------|-----------|----------|
| **OpenAI** | 详细结构化输出要求 | ~100 tokens |
| **DeepSeek** | 简洁版，针对代码优化 | ~80 tokens |
| **Moonshot** | 超简洁指令 | ~70 tokens |
| **智谱AI** | 规范化格式 | ~85 tokens |

### 提示词优化效果
- **节省30% Token**: 系统提示词从150+字符精简到约100字符
- **明确输出格式**: "直接输出Markdown"指令，避免无用的前缀/后缀
- **平台适配**: 根据不同平台特性调整提示词策略
